...
public class CurrencyTradingBaseObject {
	private static String logPath = "./log/TestLog/MarketPriceBackFeed/";
	private static String logName1 = "MissingRecord_DBI_CurrencyTradingPrice(BaseObject).log";
	private static String logName2 = "ContentChecking_DBI_CurrencyTradingPrice(BaseObject).log";
...
//Table DBI_CURRENCYTRADING				
		switch(tsType){									
		case tsTenforeCurrencyTradingPrice:
//CreateLogFile
			try {
				CustomizedLog.creatCusomizedLogFile(logPath, logName1, "MissingRecord_CurrencyTradingPrice(BaseObject)");
				CustomizedLog.creatCusomizedLogFile(logPath, logName2, "ContentChecking_CurrencyTradingPrice(BaseObject)");
...
//batch IDs testing once
				...
				tsdbValueMap = CurrencyTradingBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				...
				try {
					netezzaValueMap = CurrencyTradingBaseObjectUtil.getFullValueFromNetezza(tsType, Database.Netezza1, sublist, "DBI_CURRENCYTRADING");
				} catch (SQLException e) {
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
		...
		case tsForexSpotExchangeRate:
			...
			perfIdContainer = getSubList(perfIdList, bufferSize);
			int countTsForexSpotExchangeRate =0;
			for(List<String> sublist:perfIdContainer){
				Map<String,List<String>> dateValueListMapTsdb = new HashMap<String,List<String>>();
				Map<String,List<String>> dateValueListMapNetezza = new HashMap<String,List<String>>();
				List<String> valueListTsdb = new ArrayList<String>();
				...
				countTsForexSpotExchangeRate++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsForexSpotExchangeRate*bufferSize,size - countTsForexSpotExchangeRate*bufferSize);
				long startTime1 = System.currentTimeMillis();
				tsdbValueMap = CurrencyTradingBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);			
				long endTime1 = System.currentTimeMillis() - startTime1;
				System.out.println("[INFO]Loading data from TSDB has finished!Total cost: " + endTime1/(1000*60) + " min");
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
	...
	public static void main(String[] args) throws TSException, Exception{
		List<String> individualPerfIdtsTenforeCurrencyTradingPriceList = new ArrayList<String>();
		List<String> individualPerfIdtsForexSpotExchangeRateList = new ArrayList<String>();
		
		String tableName = "DBI_CURRENCYTRADING";
		String createdOnTime = "2014-06-16";
		int limitCount = 200;
		List<String> performanceIdListEODMode = new ArrayList<String>();

//EOD MODE(1575 Unclear Business)
		performanceIdListEODMode = getUpdatedPerformanceIdListEODMode(tableName, createdOnTime);
		if(!performanceIdListEODMode.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsTenforeCurrencyTradingPrice,performanceIdListEODMode,limitCount);
			compareUpdatedData(TsBlobDataTypeBaseObject.tsForexSpotExchangeRate,performanceIdListEODMode,limitCount);
		}else{
			System.out.println("[WARN]PerformanceId(EOD MODE) list is empty,ignore this QA check!");
		}
		
//Individual PerformanceId mode
		individualPerfIdtsTenforeCurrencyTradingPriceList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsTenforeCurrencyTradingPrice.TsType,limitCount);
		if(!individualPerfIdtsTenforeCurrencyTradingPriceList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsTenforeCurrencyTradingPrice, individualPerfIdtsTenforeCurrencyTradingPriceList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsTenforeCurrencyTradingPrice list is empty,ignore this QA check!");
		}
		
		individualPerfIdtsForexSpotExchangeRateList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsForexSpotExchangeRate.TsType,limitCount);
		if(!individualPerfIdtsForexSpotExchangeRateList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsForexSpotExchangeRate, individualPerfIdtsForexSpotExchangeRateList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsForexSpotExchangeRate list is empty,ignore this QA check!");
		}
	}
}
