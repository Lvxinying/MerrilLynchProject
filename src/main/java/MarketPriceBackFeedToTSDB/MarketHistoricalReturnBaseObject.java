...
public class MarketHistoricalReturnBaseObject {
	private static String logPath = "./log/TestLog/MarketPriceBackFeed/";
	private static String logName1 = "MissingRecord_DBI_MarketHistoricalReturn(BaseObject).log";
	private static String logName2 = "ContentChecking_DBI_MarketHistoricalReturn(BaseObject).log";
...
//Table DBI_MARKETHISTORICALRETURN				
		switch(tsType){
		case tsMonthlyMarketReturn:
//CreateLogFile
			try {
				CustomizedLog.creatCusomizedLogFile(logPath, logName1, "MissingRecord_MarketHistoricalReturn(BaseObject)");
				CustomizedLog.creatCusomizedLogFile(logPath, logName2, "ContentChecking_MarketHistoricalReturn(BaseObject)");
			} catch (IOException e1) {
				System.out.println("[ERROR]Creating Log file error,info: " +e1.getMessage());
			}
			System.out.println("[INFO]Begin to test values of MarketHistoricalReturn between Netezza and TsDB");
			System.out.println("[INFO]TsDB Mapping type:BaseObject--> TsDB env : STG");
			System.out.println("[INFO]Netezza env:Dev env--> DB Name:MSNZSTG");
			System.out.println("[INFO]Total PerformanceId count is :  "+perfIdList.size());
			
			perfIdContainer = getSubList(perfIdList, bufferSize);
			int countTsMonthlyMarketReturn =0;
			for(List<String> sublist:perfIdContainer){
				...
				countTsMonthlyMarketReturn++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsMonthlyMarketReturn*bufferSize,size - countTsMonthlyMarketReturn*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketHistoricalReturnBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);
				...
				try {
					netezzaValueMap = MarketHistoricalReturnBaseObjectUtil.getFullValueFromNetezza(tsType, Database.Netezza2, sublist, "DBI_MARKETHISTORICALRETURN");
				} catch (SQLException e) {
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
		...
		case tsQuarterlyMarketReturn:
			...
			perfIdContainer = getSubList(perfIdList, bufferSize);
			int countTsQuarterlyMarketReturn =0;
			for(List<String> sublist:perfIdContainer){
				Map<String,List<String>> dateValueListMapTsdb = new HashMap<String,List<String>>();
				Map<String,List<String>> dateValueListMapNetezza = new HashMap<String,List<String>>();
				List<String> valueListTsdb = new ArrayList<String>();
				...
				countTsQuarterlyMarketReturn++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsQuarterlyMarketReturn*bufferSize,size - countTsQuarterlyMarketReturn*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketHistoricalReturnBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				long endTime1 = System.currentTimeMillis() - startTime1;
				System.out.println("[INFO]Loading data from TSDB has finished!Total cost: " + endTime1/(1000*60) + " min");
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
		...
		case tsAnnualMarketReturn:
			...
			perfIdContainer = getSubList(perfIdList, bufferSize);
			int countTsAnnualMarketReturn =0;
			for(List<String> sublist:perfIdContainer){
				Map<String,List<String>> dateValueListMapTsdb = new HashMap<String,List<String>>();
				Map<String,List<String>> dateValueListMapNetezza = new HashMap<String,List<String>>();
				List<String> valueListTsdb = new ArrayList<String>();
				...
				countTsAnnualMarketReturn++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsAnnualMarketReturn*bufferSize,size - countTsAnnualMarketReturn*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketHistoricalReturnBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				long endTime1 = System.currentTimeMillis() - startTime1;
				System.out.println("[INFO]Loading data from TSDB has finished!Total cost: " + endTime1/(1000*60) + " min");
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
	...
	public static void main(String[] args) throws TSException, Exception{		
		List<String> individualPerfIdtsMonthlyMarketReturnList = new ArrayList<String>();
		List<String> individualPerfIdtsQuarterlyMarketReturnList = new ArrayList<String>();
		List<String> individualPerfIdtsAnnualMarketReturnList = new ArrayList<String>();
		
		
		String tableName = "DBI_MARKETHISTORICALRETURN";
...
//EOD MODE
		...
		if(!performanceIdListEODMode.isEmpty()){			
			compareUpdatedData(TsBlobDataTypeBaseObject.tsMonthlyMarketReturn,performanceIdListEODMode,limitCount);
			compareUpdatedData(TsBlobDataTypeBaseObject.tsQuarterlyMarketReturn, performanceIdListEODMode,limitCount);
			compareUpdatedData(TsBlobDataTypeBaseObject.tsAnnualMarketReturn,performanceIdListEODMode,limitCount);		
		}else{
			System.out.println("[WARN]PerformanceId(EOD MODE) list is empty,ignore this QA check!");
		}
		
//Individual PerformanceId mode
		individualPerfIdtsMonthlyMarketReturnList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsMonthlyMarketReturn.TsType,limitCount);
		if(!individualPerfIdtsMonthlyMarketReturnList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsMonthlyMarketReturn, individualPerfIdtsMonthlyMarketReturnList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsMonthlyMarketReturn list is empty,ignore this QA check!");
		}
		
		individualPerfIdtsQuarterlyMarketReturnList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsQuarterlyMarketReturn.TsType,limitCount);
		if(!individualPerfIdtsQuarterlyMarketReturnList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsQuarterlyMarketReturn,individualPerfIdtsQuarterlyMarketReturnList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsQuarterlyMarketReturn list is empty,ignore this QA check!");
		}
		
		individualPerfIdtsAnnualMarketReturnList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsAnnualMarketReturn.TsType,limitCount);
		if(!individualPerfIdtsAnnualMarketReturnList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsAnnualMarketReturn,individualPerfIdtsAnnualMarketReturnList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsAnnualMarketReturn list is empty,ignore this QA check!");
		}
	}
}
