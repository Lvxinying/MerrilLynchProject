...
public class MarketPriceForWarrantsBaseObject {
	private static String logPath = "./log/TestLog/MarketPriceBackFeed/";
	private static String logName1 = "MissingRecord_DBI_MARKETPRICEFORWARRANTS(BaseObject).log";
	private static String logName2 = "ContentChecking_DBI_MARKETPRICEFORWARRANTS(BaseObject).log";
...
//CreateLogFile
			try {
				CustomizedLog.creatCusomizedLogFile(logPath, logName1, "MissingRecord_MarktPriceForWarrants(BaseObject)");
				CustomizedLog.creatCusomizedLogFile(logPath, logName2, "ContentChecking_MarktPriceForWarrants(BaseObject)");
			} catch (IOException e1) {
				System.out.println("[ERROR]Creating Log file error,info: " +e1.getMessage());
			}
			System.out.println("[INFO]Begin to test values of MarketPriceForWarrants between Netezza and TsDB");
			...
			int countTsWarrantMarketPrice =0;
			for(List<String> sublist:perfIdContainer){
				...
				countTsWarrantMarketPrice++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsWarrantMarketPrice*bufferSize,size - countTsWarrantMarketPrice*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketPriceForWarrantsBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				...
				try {
					netezzaValueMap = MarketPriceForWarrantsBaseObjectUtil.getFullValueFromNetezza(tsType, Database.Netezza1, sublist, "DBI_MARKETPRICEFORWARRANTS");
				} catch (SQLException e) {
...
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsMarketPriceCurrencyHistory*bufferSize,size - countTsMarketPriceCurrencyHistory*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketPriceForWarrantsBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				long endTime1 = System.currentTimeMillis() - startTime1;
				System.out.println("[INFO]Loading data from TSDB has finished!Total cost: " + endTime1/(1000*60) + " min");
...
//Get netezza date list
					...
					}else if(dateValueListMapNetezza.isEmpty()){
						CustomizedLog.writeCustomizedLogFile(logPath+logName1, "[ERROR]Netezza have no related date data!");
						CustomizedLog.writeCustomizedLogFile(logPath+logName1, "TsType NAME is: " + tsType);
						CustomizedLog.writeCustomizedLogFile(logPath+logName1, "Performance ID is: " + keyPerfId);
						for(String strDate:dateValueListMapTsdb.keySet()){
							CustomizedLog.writeCustomizedLogFile(logPath+logName1, "Missing date in Netezza is: "+strDate);
						}			
						CustomizedLog.writeCustomizedLogFile(logPath+logName1, "================================================");
					}
...
//CreateLogFile
			...
			perfIdContainer = getSubList(perfIdList, bufferSize);
			int countTsWarrantTradingVolume =0;
			for(List<String> sublist:perfIdContainer){
				Map<String,List<String>> dateValueListMapTsdb = new HashMap<String,List<String>>();
				Map<String,List<String>> dateValueListMapNetezza = new HashMap<String,List<String>>();
				List<String> valueListTsdb = new ArrayList<String>();
				...
				countTsWarrantTradingVolume++;
//batch IDs testing once
				System.out.printf("[INFO]Start to load %d PerformanceId... ... \n",bufferSize);
				System.out.printf("[INFO]Prepare to test %d PerformanceIds, remaining %d PerformanceIds \n", countTsWarrantTradingVolume*bufferSize,size - countTsWarrantTradingVolume*bufferSize);
				long startTime1 = System.currentTimeMillis();				
				tsdbValueMap = MarketPriceForWarrantsBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				long endTime1 = System.currentTimeMillis() - startTime1;
				System.out.println("[INFO]Loading data from TSDB has finished!Total cost: " + endTime1/(1000*60) + " min");
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
							...
							if(keyDate!=null && dateValueListMapTsdb.containsKey(keyDate)){
								...
								if(valueListNetezza.isEmpty() || valueListNetezza == null){
									...
									for(String strNetezza:valueListNetezza){
										strTsDB = valueListTsdb.get(0);
										Double doubleValueTsDB = Double.valueOf(strTsDB);
										Double doubleValueNetezza = Double.valueOf(strNetezza);
										if(!strNetezza.equals("NullValue") && !strTsDB.equals("NaN")){
											if(!doubleValueNetezza.equals(doubleValueTsDB)){
												CustomizedLog.writeCustomizedLogFile(logPath+logName2, "[ERROR]Value not match between Netezza and TsDB!");
												CustomizedLog.writeCustomizedLogFile(logPath+logName2, "TsType NAME is: " + tsType);
												CustomizedLog.writeCustomizedLogFile(logPath+logName2, "Performance ID is: "+keyPerfId);
												CustomizedLog.writeCustomizedLogFile(logPath+logName2, "Mismatch date is: "+keyDate);
	...
	public static void main(String[] args) throws TSException, Exception{		
		List<String> individualPerfIdtsWarrantMarketPriceList = new ArrayList<String>();
		List<String> individualPerfIdtsWarrantMarketPriceCurrencyHistoryList = new ArrayList<String>();
		List<String> individualPerfIdtsWarrantTradingVolumeList = new ArrayList<String>();
		
		
		String tableName = "DBI_MARKETPRICEFORWARRANTS";
...
//EOD MODE
		if(!performanceIdListEODMode.isEmpty()){			
			...
			if(!performanceIdListEODMode.isEmpty()){
				compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantMarketPrice,performanceIdListEODMode,limitCount);
				compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantMarketPriceCurrencyHistory, performanceIdListEODMode,limitCount);
				compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantTradingVolume,performanceIdListEODMode,limitCount);
			}else{
...
//Individual PerformanceId mode
		individualPerfIdtsWarrantMarketPriceList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsWarrantMarketPrice.TsType,limitCount);
		if(!individualPerfIdtsWarrantMarketPriceList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantMarketPrice, individualPerfIdtsWarrantMarketPriceList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsWarrantMarketPrice list is empty,ignore this QA check!");
		}
		
		individualPerfIdtsWarrantMarketPriceCurrencyHistoryList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsWarrantMarketPriceCurrencyHistory.TsType,limitCount);
		if(!individualPerfIdtsWarrantMarketPriceCurrencyHistoryList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantMarketPriceCurrencyHistory,individualPerfIdtsWarrantMarketPriceCurrencyHistoryList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsWarrantMarketPrice list is empty,ignore this QA check!");
		}
		
		individualPerfIdtsWarrantTradingVolumeList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsWarrantTradingVolume.TsType,limitCount);
		if(!individualPerfIdtsWarrantTradingVolumeList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsWarrantTradingVolume,individualPerfIdtsWarrantTradingVolumeList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsWarrantMarketPrice list is empty,ignore this QA check!");
		}
	}
