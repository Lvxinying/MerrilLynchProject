...
public class UnsplitDistributionBaseObjectUtil {
	...
	private static String logName = "404NotFoundDataInTSDB_DBI_UnsplitDistribution(BaseObject).log";
...
//Connect to DB	
		...
		for(String perfIdStr : perfIdList){
			...
			switch(tsType){
			case tsCashDividend:
//特殊逻辑，distributiontype = 11				
				sqlToGetValueInCashDividend = "SELECT ExcludingDate,TotalDistribution FROM " + tableName +" WHERE PERFORMANCEID = '"
				+ perfIdStr + "' AND DistributionType = 11 AND ExcludingDate IS NOT NULL AND TotalDistribution IS NOT NULL ORDER BY ExcludingDate DESC";
//Getting trouble, re-connect to DB				
			...
			case tsCapitalGain:
//特殊逻辑，只选择DistributionType = 12的				
				sqlToGetValueInCapitalGain = "SELECT ExcludingDate,PayDate,TotalDistribution FROM " + tableName +" WHERE PERFORMANCEID = '"
				+ perfIdStr + "' AND DistributionType = 12 AND ExcludingDate IS NOT NULL AND PayDate IS NOT NULL AND TotalDistribution IS NOT NULL ORDER BY ExcludingDate DESC";
//Getting trouble, re-connect to DB				
				if(con.isClosed() || con == null){
					connectToDb(database);
				}				
