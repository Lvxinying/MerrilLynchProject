...
public class InvestmentPriceSplitAdjustedUtil {
	...
	private static String logName = "404NotFoundDataInTSDB_DBI_InvestmentPrice(SplitAdjusted).log";
...
//Connect to DB	
		...
		for(String perfIdStr : perfIdList){
			...
			switch(tsType){
			case tsMarketPrice:
//特殊逻辑，选择InvestmentType = 130，PriceType = 2				
				...
				+ perfIdStr + "' AND EffectiveDate IS NOT NULL AND Price IS NOT NULL " +
						"AND InvestmentType = 130 AND PriceType = 2 ORDER BY EffectiveDate DESC";
