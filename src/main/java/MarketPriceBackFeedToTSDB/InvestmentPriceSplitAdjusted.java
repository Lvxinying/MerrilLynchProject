...
public class InvestmentPriceSplitAdjusted {
	private static String logPath = "./log/TestLog/MarketPriceBackFeed/";
	private static String logName1 = "MissingRecord_DBI_InvestmentPrice(SplitAdjusted).log";
	private static String logName2 = "ContentChecking_DBI_InvestmentPrice(SplitAdjusted).log";
...
//Table DBI_INVESTMENTPRICE				
		switch(tsType){
		case tsMarketPrice:
			//CreateLogFile
			try {
				CustomizedLog.creatCusomizedLogFile(logPath, logName1, "MissingRecord_InvestmentPrice(SplitAdjusted)");
				CustomizedLog.creatCusomizedLogFile(logPath, logName2, "ContentChecking_InvestmentPrice(SplitAdjusted)");
			} catch (IOException e1) {
				System.out.println("[ERROR]Creating Log file error,info: " +e1.getMessage());
			}
			System.out.println("[INFO]Begin to test values of InvestmentPrice between Netezza and TsDB");
	...
	//batch IDs testing once
				...
				tsdbValueMap = InvestmentPriceSplitAdjustedUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);				
				...
				try {
					netezzaValueMap = InvestmentPriceSplitAdjustedUtil.getFullValueFromNetezza(tsType, Database.Netezza1, sublist, "DBI_INVESTMENTPRICE");
				} catch (SQLException e) {
	...
	public static void main(String[] args) throws TSException, Exception{
		...
		String tableName = "DBI_INVESTMENTPRICE";
...
//EOD MODE
		...
		if(!performanceIdListEODMode.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsMarketPrice,performanceIdListEODMode,limitCount);
		}else{
