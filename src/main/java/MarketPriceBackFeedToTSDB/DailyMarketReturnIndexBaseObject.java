...
public class DailyMarketReturnIndexBaseObject {
	private static String logPath = "./log/TestLog/MarketPriceBackFeed/";
	private static String logName1 = "MissingRecord_DBI_DailyMarketReturnIndex(BaseObject).log";
	private static String logName2 = "ContentChecking_DBI_DailyMarketReturnIndex(BaseObject).log";
...
//Table DBI_MARKETPRICE				
		...
		case tsDailyMarketReturnIndex:
			//CreateLogFile
			try {
				CustomizedLog.creatCusomizedLogFile(logPath, logName1, "MissingRecord_DailyMarketReturnIndex(BaseObject)");
				CustomizedLog.creatCusomizedLogFile(logPath, logName2, "ContentChecking_DailyMarketReturnIndex(BaseObject)");
...
//batch IDs testing once
				...
				tsdbValueMap = DailyMarketReturnIndexBaseObjectUtil.getFullValueFromTsDB(tsType, tsType.TsType, sublist);
				...
				try {
					netezzaValueMap = DailyMarketReturnIndexBaseObjectUtil.getFullValueFromNetezza(tsType, Database.Netezza1, sublist, "DBI_DAILYMARKETRETURNINDEX");
				} catch (SQLException e) {
...
//Netezza和TsDB中的日期格式不同，需要转换一下							
	...
	public static void main(String[] args) throws TSException, Exception{
		List<String> individualPerfIdtsDailyMarketReturnIndexList = new ArrayList<String>();
...
//Case1:ET		
		...
		if(!performanceIdListEODModeET.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex,performanceIdListEODModeET,limitCount);
		}else{
...
//Case2:CE		
		...
		if(!performanceIdListEODModeCE.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex,performanceIdListEODModeCE,limitCount);
		}else{
...
//Case3:IN		
		...
		if(!performanceIdListEODModeIN.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex,performanceIdListEODModeIN,limitCount);
		}else{
...
//Case4:Equity
		...
		if(!performanceIdListEODModeEQUITY.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex,performanceIdListEODModeEQUITY,limitCount);
		}else{
...
//Individual PerformanceId mode
		individualPerfIdtsDailyMarketReturnIndexList = getUpdatedPerformanceIdListIndividualMode(tableName,TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex.TsType,limitCount);
		if(!individualPerfIdtsDailyMarketReturnIndexList.isEmpty()){
			compareUpdatedData(TsBlobDataTypeBaseObject.tsDailyMarketReturnIndex, individualPerfIdtsDailyMarketReturnIndexList,limitCount);
		}else{
			System.out.println("[WARN]Individual PerfId TsDailyMarketReturnIndex list is empty,ignore this QA check!");
		}		
	}
}
